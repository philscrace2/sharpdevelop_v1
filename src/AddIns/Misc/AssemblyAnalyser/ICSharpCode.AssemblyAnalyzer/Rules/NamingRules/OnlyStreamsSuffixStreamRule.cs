...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class OnlyStreamsSuffixStreamRule : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyStreamsSuffixStream.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyStreamsSuffixStream.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (!typeof(System.IO.Stream).IsAssignableFrom(type) && type.Name.EndsWith("Stream")) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyStreamsSuffixStream.Resolution}", type.FullName, new string[,] { { "TypeName", type.FullName }});
			}
			return null;
		}
	}
