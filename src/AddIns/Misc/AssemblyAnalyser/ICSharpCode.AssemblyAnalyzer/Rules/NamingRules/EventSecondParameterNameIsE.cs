...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class EventSecondParameterNameIsE : AbstractReflectionRule, IEventRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventSecondParameterNameIsE.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventSecondParameterNameIsE.Details}";
			}
		}
		...
		public Resolution Check(EventInfo evnt)
		{
			...
			if (parameters.Length > 1 && parameters[1].Name != "e") {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.EventSecondParameterNameIsE.Resolution}", evnt.EventHandlerType.FullName, new string[,] { { "EventType", evnt.EventHandlerType.FullName }, { "OldParameterName", parameters[1].Name }});
			}
			return null;
		}
	}
