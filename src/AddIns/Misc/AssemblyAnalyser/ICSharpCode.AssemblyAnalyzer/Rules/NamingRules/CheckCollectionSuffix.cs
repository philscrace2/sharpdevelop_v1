...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class CheckCollectionSuffix : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.CheckCollectionSuffix.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.CheckCollectionSuffix.Details}";
			}
		}
			...
			if ((typeof(ICollection).IsAssignableFrom(type) || typeof(IEnumerable).IsAssignableFrom(type)) && !typeof(System.Collections.IDictionary).IsAssignableFrom(type)) {
				if (typeof(Queue).IsAssignableFrom(type)) {
					if (!type.Name.EndsWith("Queue")) {
						return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.CheckCollectionSuffix.Resolution1}", type.FullName, new string[,] { { "TypeName", type.FullName }} );
					}
				} else if (typeof(Stack).IsAssignableFrom(type)) {
					if (!type.Name.EndsWith("Stack")) {
						return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.CheckCollectionSuffix.Resolution2}", type.FullName, new string[,] { { "TypeName", type.FullName }});
					}
				} else {
					if (!type.Name.EndsWith("Collection")) {
						return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.CheckCollectionSuffix.Resolution3}", type.FullName, new string[,] { { "TypeName", type.FullName }});
					}
				}
			}
			return null;
