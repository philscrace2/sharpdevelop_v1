...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class EventFirstParameterNameIsSender : AbstractReflectionRule, IEventRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventFirstParameterNameIsSender.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventFirstParameterNameIsSender.Details}";
			}
		}
		...
		public Resolution Check(EventInfo evnt)
		{
			...
			if (parameters.Length > 0 && parameters[0].Name != "sender") {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.EventFirstParameterNameIsSender.Resolution}", evnt.EventHandlerType.FullName, new string[,] { { "EventType", evnt.EventHandlerType.FullName }, { "OldParameterName", parameters[0].Name }});
			}
			return null;
		}
	}
