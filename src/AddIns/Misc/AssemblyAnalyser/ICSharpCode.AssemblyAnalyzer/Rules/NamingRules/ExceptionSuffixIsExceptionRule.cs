...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class ExceptionSuffixIsExceptionRule : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.ExceptionSuffixIsException.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.ExceptionSuffixIsException.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (typeof(System.Exception).IsAssignableFrom(type) && !type.Name.EndsWith("Exception")) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.ExceptionSuffixIsException.Resolution}", type.FullName, new string[,] { { "TypeName", type.FullName }});
			}
			return null;
		}
	}
