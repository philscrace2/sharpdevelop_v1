...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class OnlyCollectionsSuffixCollectionRule : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyCollectionsSuffixCollection.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyCollectionsSuffixCollection.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (!typeof(ICollection).IsAssignableFrom(type) && !typeof(IEnumerable).IsAssignableFrom(type)) {
				if (!typeof(Queue).IsAssignableFrom(type) && type.Name.EndsWith("Queue")) {
					return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyCollectionsSuffixCollection.Resolution1}", type.FullName, new string[,] { { "TypeName", type.FullName }});
				} else if (!typeof(Stack).IsAssignableFrom(type) && type.Name.EndsWith("Stack")) {
					return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyCollectionsSuffixCollection.Resolution2}", type.FullName, new string[,] { { "TypeName", type.FullName }});
				} else if (type.Name.EndsWith("Collection")) {
					return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.OnlyCollectionsSuffixCollection.Resolution3}", type.FullName, new string[,] { { "TypeName", type.FullName }});
				}
			}
			return null;
		}
