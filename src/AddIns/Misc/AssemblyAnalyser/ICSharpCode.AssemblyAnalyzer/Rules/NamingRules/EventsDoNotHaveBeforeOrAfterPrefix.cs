...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class EventsDoNotHaveBeforeOrAfterPrefix : AbstractReflectionRule, IEventRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventsDoNotHaveBeforeOrAfterPrefix.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventsDoNotHaveBeforeOrAfterPrefix.Details}";
			}
		}
		...
		public Resolution Check(EventInfo evnt)
		{
			if (evnt.Name.StartsWith("Before")) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.EventsDoNotHaveBeforeOrAfterPrefix.Resolution1}", NamingUtilities.Combine(evnt.ReflectedType.FullName, evnt.Name), new string[,] { { "EventName", evnt.Name }, { "ReflectedType", evnt.ReflectedType.FullName }});
			} else if (evnt.Name.StartsWith("After")) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.EventsDoNotHaveBeforeOrAfterPrefix.Resolution2}", NamingUtilities.Combine(evnt.ReflectedType.FullName, evnt.Name), new string[,] { { "EventName", evnt.Name }, { "ReflectedType", evnt.ReflectedType.FullName }});	
			}
			return null;
		}
	}
