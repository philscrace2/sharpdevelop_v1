...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class EnumsHaveNoEnumSuffix : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EnumsHaveNoEnumSuffix.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EnumsHaveNoEnumSuffix.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (type.IsSubclassOf(typeof(System.Enum)) && type.Name.EndsWith("Enum")) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.EnumsHaveNoEnumSuffix.Resolution}", type.FullName, new string[,] { { "TypeName", type.FullName }});
			}
			return null;
		}
	}
