...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class DictionaryTypeSuffixIsDictionary : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.DictionaryTypeSuffixIsDictionary.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.DictionaryTypeSuffixIsDictionary.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (typeof(System.Collections.IDictionary).IsAssignableFrom(type) && !type.Name.EndsWith("Dictionary")) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.DictionaryTypeSuffixIsDictionary.Resolution}", type.FullName, new string[,] { { "TypeName", type.FullName }});
			}
			return null;
		}
	}
