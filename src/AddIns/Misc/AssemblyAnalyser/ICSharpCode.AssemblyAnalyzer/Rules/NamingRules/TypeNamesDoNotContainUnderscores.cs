...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class TypeNamesDoNotContainUnderscores : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.TypeNamesDoNotContainUnderscores.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.TypeNamesDoNotContainUnderscores.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (NamingUtilities.ContainsUnderscore(type.Name)) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.TypeNamesDoNotContainUnderscores.Resolution}", type.FullName, new string[,] { { "TypeName", type.FullName }});
			}
			return null;
		}
	}
