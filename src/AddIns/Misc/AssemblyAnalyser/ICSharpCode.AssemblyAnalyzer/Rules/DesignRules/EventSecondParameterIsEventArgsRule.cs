...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class EventSecondParameterIsEventArgsRule : AbstractReflectionRule, IEventRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventSecondParameterIsEventArgs.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.EventSecondParameterIsEventArgs.Details}";
			}
		}
		...
		public Resolution Check(EventInfo evnt)
		{
			...
			if (parameters.Length > 1 && !typeof(System.EventArgs).IsAssignableFrom(parameters[1].ParameterType)) {
				return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.EventSecondParameterIsEventArgs.Resolution}", evnt.EventHandlerType.FullName, new string[,] { { "EventType", evnt.EventHandlerType.FullName }, { "OldParameterType", parameters[1].ParameterType.FullName }});
			}
			return null;
		}
	}
