...
namespace ICSharpCode.AssemblyAnalyser.Rules
{
	...
	public class AbstractTypesPublicConstructorsRule : AbstractReflectionRule, ITypeRule
	{
		public override string Description {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.AbstractTypesPublicConstructors.Description}";
			}
		}
		
		public override string Details {
			get {
				return "${res:ICSharpCode.AssemblyAnalyser.Rules.AbstractTypesPublicConstructors.Details}";
			}
		}
		...
		public Resolution Check(Type type)
		{
			if (type.IsAbstract) {
				foreach (ConstructorInfo info in type.GetConstructors()) {
					if (info.IsPublic) {
 						return new Resolution(this, "${res:ICSharpCode.AssemblyAnalyser.Rules.AbstractTypesPublicConstructors.Resolution}", type.FullName, new string[,] { { "TypeName", type.FullName }});
					}
				}
			}
			return null;
