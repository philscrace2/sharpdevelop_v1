...
namespace ICSharpCode.SharpDevelop.FormDesigner.Services
{
	public class DesignerResourceService : System.ComponentModel.Design.IResourceService , IDisposable
	{
		...
		public DesignerResourceService(IDesignerHost host): this(host, null)
		{
		}
		
		public DesignerResourceService(IDesignerHost host, Hashtable resources)
		{
			this.host = host;
			IProjectService projectService = (IProjectService)ICSharpCode.Core.Services.ServiceManager.Services.GetService(typeof(IProjectService));
			project = projectService.CurrentSelectedProject;
		...
		public void Save()
		{
			if (resources != null) {
				foreach (DictionaryEntry entry in resources) {
					...
					fileUtilityService.ObservedSave(new NamedFileOperationDelegate(((ResourceStorage)entry.Value).Save), (string) entry.Key, FileErrorPolicy.Inform);
				}
			}
		}
		...
		protected string CalcResourceFileName(System.Globalization.CultureInfo info)
		{
			...
			if (FileName != null && FileName != String.Empty) {
				...
				resourceFileName = new StringBuilder(project.BaseDirectory);
			} else {
