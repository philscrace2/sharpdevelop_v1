...
namespace ICSharpCode.SharpDevelop.FormEditor.Commands
{
	...
	public class ShowProperties : AbstractMenuCommand
	{
		public override void Run()
		{
			IPadContent padContent = WorkbenchSingleton.Workbench.GetPad(typeof(ICSharpCode.SharpDevelop.Gui.Pads.PropertyPad));
			if (padContent != null) {
	...
	public class ShowLargeIcons : AbstractCheckableMenuCommand
	{
		...
		ComponentTray Tray {
			get {
				...
				FormDesignerDisplayBindingBase formDesigner = window.ActiveViewContent as FormDesignerDisplayBindingBase;
				if (formDesigner != null) {
					return formDesigner.DesignerHost.GetService(typeof(ComponentTray)) as ComponentTray;
				}
				return null;
