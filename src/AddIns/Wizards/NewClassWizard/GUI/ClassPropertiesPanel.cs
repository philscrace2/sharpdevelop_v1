	...
	public class ClassPropertiesPanel : MyPanelBase	{
		...
		public override bool ReceiveDialogMessage(DialogMessage message) {
			...
			if (message == DialogMessage.Finish) {
				...
				try {
					...
					if ( project != null )
						factory.Namespace = project.StandardNamespace;
					else 
						factory.Namespace = "";
					
					StringWriter declaration = null;
					StringWriter implementation = null;
					
					try {
						if (language.CanGenerateDeclaration())
						{
							declaration = new StringWriter();
						}
						
						if (language.CanGenerateImplementation())
						{
							implementation = new StringWriter();
						}
						
						factory.GenerateCode(language, declaration, implementation);
						
						if (declaration != null)
						{
							newFileCreator.SaveFile(DeclarationFileName,
						...
						{
							newFileCreator.SaveFile(ImplementationFileName,
		...
		private string DeclarationFileName {
			get {
				...
				if (language.CanGenerateDeclaration())
				{
					result = txtClassName.Text + language.GetDeclarationFileExt();
				}
				return result;
			}
		}
		...
		private string ImplementationFileName {
			get {
				...
				if (language.CanGenerateImplementation())
				{
					result = txtClassName.Text + language.GetImplementationFileExt();
				}
				return result;
			}
		}
		...
		private bool classNameIsValid()
		{
			...
			if (language.IsValidIdentifier(txtClassName.Text ) == false ) {
				errorMsg = "This is not a valid " + language.GetName() + " class name";			
			}
			else {
				if (language.CanGenerateDeclaration() && !IsFileNameValid(DeclarationFileName)) {
					errorMsg = "A file " + DeclarationFileName + " already exists in this project";
				}
				else if (language.CanGenerateImplementation() && !IsFileNameValid(ImplementationFileName)) {
					errorMsg = "A file " + ImplementationFileName + " already exists in this project";
				}
			}
